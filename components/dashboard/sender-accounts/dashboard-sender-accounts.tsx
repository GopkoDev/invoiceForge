...
export function DashboardSenderAccounts({
  ...
  const [expandedSenders, setExpandedSenders] = React.useState<Set<string>>(
    new Set()
  );

  // Auto-expand first sender on mount
  React.useEffect(() => {
    if (senderAccounts.length > 0 && expandedSenders.size === 0) {
      setExpandedSenders(new Set([senderAccounts[0].senderProfileId]));
    }
